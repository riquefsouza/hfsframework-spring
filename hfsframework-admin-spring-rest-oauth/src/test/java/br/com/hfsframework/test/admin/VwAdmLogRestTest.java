...
/**
 * The Class VwAdmLogTest.
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = Aplicacao.class)
@WebAppConfiguration
@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class VwAdmLogRestTest extends BaseTest {
	
	/** The Constant log. */
	private static final Logger log = LoggerFactory.getLogger(VwAdmLogRestTest.class);
    ...
    public void a_insert() throws Exception {
    	this.vwAdmLogList.stream().forEach(bean -> 
	        {
				try {
					mockMvc.perform(post("/vwAdmLogs")
    ...
    public void b_listar() throws Exception {
        mockMvc.perform(get("/vwAdmLogs")
        		.header("Authorization", "Bearer " + accessToken).accept(contentType))
                .andExpect(status().isOk())
                .andExpect(content().contentType(contentType))
                .andExpect(jsonPath("$", hasSize(this.vwAdmLogList.size())))
                .andExpect(jsonPath("$[0].id", is(this.vwAdmLogList.get(0).getId().intValue())))
                .andExpect(jsonPath("$[0].tabela", is(this.vwAdmLogList.get(0).getTabela())))
                .andExpect(jsonPath("$[1].id", is(this.vwAdmLogList.get(1).getId().intValue())))
                .andExpect(jsonPath("$[1].tabela", is(this.vwAdmLogList.get(1).getTabela())));
    }
    ...
    public void c_search() throws Exception {
        ...
        {
			try {
		        mockMvc.perform(get("/vwAdmLogs/" + bean.getId())
		        		.header("Authorization", "Bearer " + accessToken).accept(contentType))
		                .andExpect(status().isOk())
		                .andExpect(content().contentType(contentType))
    ...
    public void d_update() throws Exception {
    	this.vwAdmLogList.stream().forEach(bean -> 
	        {
				try {
					mockMvc.perform(put("/vwAdmLogs/"+bean.getId())
							.header("Authorization", "Bearer " + accessToken).accept(contentType)
					        .content(this.json(bean))
					        .contentType(contentType))
					        .andExpect(status().isOk());
    ...
    public void e_delete() throws Exception {
    	this.vwAdmLogList.stream().forEach(bean -> 
    		{
				try {
			        mockMvc.perform(delete("/vwAdmLogs/"+bean.getId())
			        		.header("Authorization", "Bearer " + accessToken).accept(contentType))
